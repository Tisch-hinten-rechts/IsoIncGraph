...
def execute_isoform(graph, generic_feature, beginning="("):
    ...
    qualifiers_vals = [x[1].get("qualifiers") for x in edge_list]
    isoforms_vals = [x[1].get("isoforms") for x in edge_list]

    # Ensure attributes exist (optional safety check)
    if "qualifiers" not in graph.es.attributes():
        graph.es["qualifiers"] = [None] * graph.ecount()
    if "isoforms" not in graph.es.attributes():
        graph.es["isoforms"] = [None] * graph.ecount()
