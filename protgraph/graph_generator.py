...
from protgraph.scripts.peptide_search import add_peptides_to_graph, contains_path_string, get_peptides
    ...
    with Exporters(**kwargs) as graph_exporters:
        ...
        while True:
            ...
            if kwargs["search_graph"]:
                peptides, metadata = get_peptides(entry.accessions[0], **kwargs)
                add_peptides_to_graph(graph, peptides, metadata, kwargs["intensity"], kwargs["merge_peptides"], kwargs["count_peptides"], kwargs["overlapping_intensities"] if kwargs["overlapping_intensities"] else False)
