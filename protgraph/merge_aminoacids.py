...
def merge_aminoacids(graph_entry):
    ...
    for idcs in complete_chain:
        ...
        if "signal" in sorted_nodes_attrs[0]:
            m_signal = [
                y
                for x in sorted_edges
                if x.attributes().get("signal") is not None
                for y in (x.attributes()["signal"] if isinstance(x.attributes()["signal"], list) else [x.attributes()["signal"]])
            ]
        else:
            m_signal = None
